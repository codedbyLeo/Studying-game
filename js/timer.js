(async function () {
  const user = await sqInitShell("timer");
  if (!user) return;
  ...
  if (activeEvent && eventBanner) { eventBanner.style.display = "block"; eventBanner.innerHTML = `<strong>⚡ ${sqEscape(activeEvent.name)}</strong> · ${Number(activeEvent.multiplier)}× XP right now!`; }
  ...
  function renderStopwatch() {
    ...
    xpEl.textContent = `${Math.floor(seconds / 60)} XP so far${activeEvent ? ` · ${Number(activeEvent.multiplier)}× event` : ""}`;
  }
  renderStopwatch();
  ...
  document.getElementById("tw-finish").addEventListener("click", async () => {
    ...
    const { session, unlocked } = await SQ.addSession(user.id, {
