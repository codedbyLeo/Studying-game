(async function () {
  const user = await sqInitShell("dashboard");
  if (!user) return;

  const sessions = SQ.getSessions(user.id);
  const activeEvent = await SQ.getActiveEvent();
  if (await SQ.isAdmin()) { const h=document.querySelector(".page-header .flex"); if(h && !document.getElementById("admin-link")) h.insertAdjacentHTML("beforeend", `<a id="admin-link" class="btn btn-ghost" href="admin.html"><i data-lucide="wand-sparkles"></i> Game control</a>`); }
